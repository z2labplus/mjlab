...
class MahjongInitialHandDeducer:
    ...
    def validate_input_data(self):
        """验证输入数据格式"""
        required_keys = ['actions', 'final_hands']
        for key in required_keys:
            if key not in self.game_data:
                raise ValueError(f"游戏数据缺少必要字段: {key}")
        
        # 检查是否有已知的初始手牌
        if 'initial_hands' not in self.game_data:
            raise ValueError("游戏数据缺少initial_hands字段")
        
        known_players = list(self.game_data['initial_hands'].keys())
    ...
    def get_peng_self_tiles(self, player_id):
        """获取碰牌中自己的牌"""
        final_hands = self.game_data['final_hands']
    ...
    def deduce_all_unknown_hands(self):
        ...
        for player_id_str in known_players:
            results[player_id_str] = self.game_data['initial_hands'][player_id_str]
        ...
        for player_id in range(4):  # 麻将通常是4个玩家
            if player_id not in known_player_ids:
                # 检查该玩家是否存在于最终手牌中
                if str(player_id) in self.game_data['final_hands']:
    ...
    def create_complete_replay(self, deduced_hands, output_file):
        ...
        complete_replay = {
            ...
            "final_hands": self.game_data.get('final_hands', {}),
        ...
        # 填入初始手牌数据
        known_players = list(self.game_data.get('initial_hands', {}).keys())
